...
const loadCart = async (req, res) => {
  try {
    ...
    //   return total;
    // }, 0);
    const cartItem = await User.findOne({ email: email }).populate(
      "carts.products"
    );
  ...
  try {
      ...
      for (const offer of offersList) {
        if (
          ...
          categoryOffer =
            ...
            (product.regularPrice * offer.offer) / 100 -
    ...
    if (existingProduct) {
      ...
      cart.products.push({
        ...
        totalPrice:
          categoryOffer * quantity ||
          (product.regularPrice - product.productOffer) * quantity,
...
const updateCart = async (req, res) => {
  try {
    ...
    const product = await Products.findById(
      cart.products[cartItemIndex].productId
    );
    ...
    cart.products[cartItemIndex].totalPrice =
      cart.products[cartItemIndex].quantity *
      cart.products[cartItemIndex].price;
...
const addCart = async (req, res) => {
  try {
    ...
    const availableSize = Object.keys(product.size).filter(
      (size) => product.size[size] > 0
    );
    ...
    const randomSize =
      availableSize[Math.floor(Math.random() * availableSize.length)];
    ...
    const existingProduct = cart.products.find(
      (item) =>
        item.productId.toString() === productId && item.size === randomSize
    );
