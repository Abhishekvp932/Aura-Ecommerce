...
function genarateRefferalCode(length = 8) {
  ...
  for (let i = 0; i < length; i++) {
    refferalCode += characters.charAt(
...
const signup = async (req, res) => {
  ...
  (req.session.userLoged = true), res.render("login", { mg: req.flash("err") });
};
...
const loadAddress = async (req, res) => {
  try {
    ...
    const email = req.session.userEmail;
    const user = await User.findOne({ email: email });
    const addres = await Address.findOne({ userId: user._id }).populate(
      "addresses"
    );
...
const loadEditeAddress = async (req, res) => {
  try {
    ...
    const address = addressDocument.addresses.find(
